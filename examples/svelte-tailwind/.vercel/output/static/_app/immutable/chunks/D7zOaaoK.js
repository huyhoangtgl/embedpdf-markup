import{s as r,W as o}from"./rYRUZYAT.js";function i(t,n){const e=new Worker(URL.createObjectURL(new Blob([`var Rotation = /* @__PURE__ */ ((Rotation2) => {
...
class VietnameseFontSystem {
   ...
   */
  addFontMapping(charset, fontUrl, fontName) {
    if (!this.initialized) {
  ...
  getFontData(fontUrl) {
    ...
    return (entry == null ? void 0 : entry.loaded) ? entry.data : null;
  }
  /**
   ...
   */
  isFontLoaded(fontUrl) {
    var _a;
    return ((_a = this.fontCache.get(fontUrl)) == null ? void 0 : _a.loaded) ?? false;
  }
  /**
   * Get font mapping for a specific charset
   */
  getFontForCharset(charset) {
...
class PdfiumEngine {
  ...
  addImageObject(docPtr, page, pagePtr, annotationPtr, rect, imageData) {
    ...
    const pagePos = this.convertDevicePointToPagePoint(page, {
      ...
      // Use updated height after rotation
    });
   ...
   * @private
   */
  rotateImageData(imageData, rotation) {
   ...
   * @param fontUrl - URL to the font file (e.g., '/fonts/NotoSans-Regular.ttf')
   * @param fontName - Display name for the font
   * @public
   */
  configureFallbackFont(charset, fontUrl, fontName) {
    this.fontSystem.addFontMapping(charset, fontUrl, fontName);
   ...
   * 
   * @param fontUrl - URL to the font file to preload
   * @returns Promise that resolves to true if font was loaded successfully
   * @public
   */
  async preloadFont(fontUrl) {
    const result = await this.fontSystem.preloadFont(fontUrl);
    if (result) {
      this.logger.debug(LOG_SOURCE, LOG_CATEGORY, \`Successfully preloaded font: \${fontUrl}\`);
    } else {
      this.logger.warn(LOG_SOURCE, LOG_CATEGORY, \`Failed to preload font: \${fontUrl}\`);
    }
    return result;
  }
